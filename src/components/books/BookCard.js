...
export default function BookCard({ book }) {
    ...
    return (
        <Link href={`/books/${book.id}`} className="group relative block">
            <div className="aspect-[2/3] rounded-xl overflow-hidden mb-3 shadow-lg bg-gray-800 relative transform transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-indigo-500/20 group-hover:shadow-2xl">
                {book.cover ? (
                    <Image
                        src={book.cover}
                        alt={book.title}
                ...
                {/* Hover Info */}
                <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300 bg-black/60 backdrop-blur-md">
            ...
            <h3 className="font-bold text-gray-100 truncate group-hover:text-indigo-400 transition-colors">{book.title}</h3>
